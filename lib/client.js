...
window.__ModuleLoader__.load({
  ...
  factory: (require) => {
    ...
    const styleKernel = (function () {
      ...
      return { STYLE_KEY_ORDER, PALETTE, NODE_SHAPES, NODE_SHAPE_STYLE, DASH_KINDS, ARROW_KINDS, SIDES, DEFAULT_FILL, DEFAULT_STROKE, DEFAULT_FONT, DEFAULT_NODE_STYLE, DEFAULT_EDGE_STYLE, parseStyle, formatStyle, styleGet, styleSet, stylePatch, styleIsEmpty, styleNumber, nodeShapeFromStyle, styleFromNodeShape, styleWithNodeShape, colorsFromStyle, colorNameFromStyle, styleWithColorName, styleWithColors, dashFromStyle, styleWithDash, dashPatternFromStyle, arrowFromStyle, styleWithArrow, sideFromStyle, fractionFromStyle, styleWithSide, sideFromFraction, edgeStyleValueFromStyle, isOrthogonalEdgeStyle, jettyFromStyle, styleWithJetty, avoidFromStyle, styleWithAvoid, normalizePoint, normalizePoints, pointsEqual, edgeTerminalId, edgeFreePoint, normalizeDrawioDoc, snapDocGeometry }
    })()
    ...
    /** 边自己的文字落在哪：**drawio 的规则** —— 沿折线的弧长中点。
     *
     * drawio 的边标签落点由 `mxGraphView.updateEdgeLabelOffset` 决定：边自己的 `mxGeometry` 是
     * `relative="1"`（我们写出来的正是这个）时走 `getPoint()`，`x/y` 缺省就是 0 ——
     * 于是 `dist = (0/2 + 0.5) * 总长` = **整条折线的中点**（不是某一段的中点，也不是包围盒中心）。
     * `x`/`y`/`offset` 是"被拖过的标签"才有的位置（见 edgeLabelPointAt 的注释）。
     *
     * 这里以前取的是"第 2 段的中点"（pts[1]→pts[2]），于是同一份文件我们和 drawio 画在不同地方：
     * 折线一拐，我们那段可能只是 24px 的引出段，文字就贴在节点边上。
     */
    function edgeLabelPosition(pts) {
      return edgeLabelPointAt(pts, 0, 0, 0, 0)
    }
    ...
    function renderDiagram(doc, mode, uid, svgRef, ui, view) {
      ...
      for (let i = 0; i < doc.edges.length; i += 1) {
        ...
        if (typeof edge.label === 'string' && edge.label.length > 0) {
          let pos = edgeLabelPosition(pts)
          // 路径退化成零长（两端落在同一点）时落点算不出来：退回起点，别让文字掉到原点 (0,0) 去。
          if (pos === null && pts.length > 0) pos = { x: pts[0].x, y: pts[0].y }
          if (pos !== null) {
            const mx = pos.x
            const my = pos.y
            ...
            children.push(React.createElement('rect', edgeFace({ key: 'edge-bg-' + i, x: mx - lw / 2, y: my - 8, width: lw, height: 13, rx: 2, fill: skin.labelBg, opacity: 0.92 })))
            children.push(
              React.createElement(
                'text',
                edgeFace({
