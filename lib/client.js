...
window.__ModuleLoader__.load({
  ...
  factory: (require) => {
    ...
    function renderDiagram(doc, mode, uid, svgRef, ui, view) {
      ...
      for (let i = 0; i < doc.edges.length; i += 1) {
        ...
        if (typeof edge.label === 'string' && edge.label.length > 0) {
          let pos = edgeLabelPosition(pts, edge)
          ...
          if (pos !== null) {
            ...
            children.push(React.createElement('rect', edgeLabelFace({ key: 'edge-bg-' + i, x: mx - lw / 2, y: my - 8, width: lw, height: 13, rx: 2, fill: skin.labelBg, opacity: 0.92 })))
            children.push(
              React.createElement(
                'text',
                edgeLabelFace({
    ...
    function CanvasView(props) {
      ...
      /** 拖标签的每一帧：把指针位置反解成 drawio 的相对位置（x 比例 / y 垂距 / 残余 offset）。 */
      function moveEdgeLabel(edgeId, point) {
        ...
        if (pts === null) return
        const pos = labelPosFor(pts, point)
        if (pos === null) return
