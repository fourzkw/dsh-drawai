  ...
  factory: (require) => {
    ...
    function routeEdge(fromBox, toBox, boxes, bounds, waypoints, sides, straight) {
      ...
      if (fromBox.id === toBox.id) {
        // 自环：坐标同样取半格（贴边那一轴 x 保持精确）。
        const x = from.x + from.w
        const cy = snapFree(from.y + from.h / 2)
        const r = 35
        return [{ x: x, y: cy - 10 }, { x: x + r, y: cy - 10 }, { x: x + r, y: cy + 10 }, { x: x, y: cy + 10 }]
      }
    ...
    function CanvasView(props) {
      ...
      if (status.kind === 'error') {
        ...
        body = React.createElement(
          ...
          React.createElement('div', { className: 'drawai-empty-hint' }, '或用「文件 → 打开…」挑一个已有的 .dshd.json'),
        )
      } else if (doc === null) {
