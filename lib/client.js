...
window.__ModuleLoader__.load({
  ...
  factory: (require) => {
     ...
     * 共线冗余那一遍不需要额外的保护名单 —— `simplifyCollinear` 只消"夹在两邻居之间"的点，
     * 折出去再折回来的那种共线点几何上必须留着（见那里的注释）。
     */
    function finalizePath(pts, snapWaypoints) {
      ...
      //     拖动那一段会直接没反应（"点了段把手不动"）。
      return simplifyCollinear(dedupePoints(snapNearAxisInPath(pts, forSnap)))
    }
    ...
    function selfLoopPath(geo, sourceSide, targetSide, jetty, waypoints) {
      ...
      pts.push(t)
      pts.push(p1)
      // 折点、两个接入点都要保住（finalizePath 只做去共线与消台阶）。
