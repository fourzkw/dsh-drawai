...
export function apply(ctx) {
  ...
  const applyTool = defineTool({
    ...
    description:
      ...
      'setStyle 的 id 可以是节点也可以是连线：节点用 shape/style/keys/w/h，连线用 dash（solid|dashed|dotted）、arrow（end 单向|both 双向|none 无箭头|start 反向）、color、exit/entry（n|e|s|w 进出侧）、jettySize（引出段长度，数字或 auto）、edgeStyle（orthogonalEdgeStyle|none）、line（straight 直线|sharp 直角折线|rounded 圆角折线|curved 曲线）、avoid（是否参与避让路由）、fontSize（字号）、clearPoints（清掉折点）。' +
