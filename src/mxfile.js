...
/**
 * 从 `<mxGraphModel>` 的属性里读出纸张尺寸（`pageWidth`/`pageHeight`）。
 *
 * 读不出来（没写 / 不是数）就返回 null —— 那是"这份文件没说"，不是 850×1100；
 * 由调用方决定要不要退回缺省（画布新建时用的就是 drawio 的 A4 竖版缺省）。
 */
function readPageSize(modelXml) {
  const w = Number(rawAttr(modelXml, 'pageWidth'))
  const h = Number(rawAttr(modelXml, 'pageHeight'))
  if (Number.isFinite(w) === false || Number.isFinite(h) === false || w <= 0 || h <= 0) return null
  return { w: w, h: h }
}
...
/**
 * 从某个单元出发，沿**文件里**的父链找它属于哪一层（读时的 `layerOf` 是同一个意思）。
 * 写回时用来判断"模型要求的层"和"文件里这一支的层"是不是同一层。
 */
function layerOfParent(info, cellOrId) {
  const first = typeof cellOrId === 'string' ? cellOrId : cellOrId === null || cellOrId === undefined ? undefined : cellOrId.id
  let parentId = first
  const seen = new Set()
  while (typeof parentId === 'string' && seen.has(parentId) === false) {
    seen.add(parentId)
    ...
    const cell = info.byId.get(parentId)
 ...
 *
 * 第三条是图层（`wantLayer`）：模型把单元放到了**另一个层**时，必须改 `parent` ——
 * 这是"移到别的层"唯一能生效的地方（不改的话文件里它还挂在老层上，读回来 layer 照旧是老的）。
 * 只认**已知的**层 id：不存在的层会写出悬空 parent，drawio 打开会丢层级。
 * 注意"容器里的单元"这一支要照旧保住：模型里的 `layer` 对容器子单元来说只是"它属于哪一层"
 * （读时沿父链得到的），并不表示它该脱离容器 —— 所以只有**层变了**才换父级。
 *
...
function writeChainOf(info, modelAbs, deletedIds, cell, wantLayer, knownLayers) {
  ...
  while (typeof parentId === 'string' && seen.has(parentId) === false) {
    seen.add(parentId)
    if (info.layerIds.has(parentId) || parentId === info.rootId) break
  ...
  if (want !== null && layerOfParent(info, parentId) !== want) return { parentId: want, offset: { x: 0, y: 0 } }
  return { parentId: parentId, offset: offset }
}
...
export function parseMxfile(text, options) {
  ...
  for (const cell of info.cells) {
    ...
    // **容器父级**：父级不是图层/root 的，说明它挂在某个容器（或别的单元）里。
    //
    // 为什么要报出来：画布按**绝对位置**显示（层级在文件里保留），于是"移动容器"在画布上
    // 并不会带走子单元，删除容器则会把子单元接到还活着的祖先上 —— AI 不知道这层关系时，
    // 这两件事都是"看起来对、其实把组弄坏了"。给一个 parent 字段，它至少能看见
    // "这个单元在哪个容器里"，要一起动就把它们一起写进 move/duplicate 的 ids。
    // 注意它只是**只读信息**：写回时父级由文件里的父链 + layer 决定（新单元一律挂在层下，
    // 不往容器里塞 —— 那需要写相对坐标，而新单元写的是绝对坐标）。
    if (typeof cell.parent === 'string' && cell.parent !== layerId && info.layerIds.has(cell.parent) === false && cell.parent !== info.rootId) {
      node.parent = String(cell.parent)
    }
    nodes.push(node)
  }
...
function planDocEdits(originalText, doc) {
  ...
  // 已知的层 id = 文件里的层 ∪ 模型里的层（模型里可能有这次刚新建的层）。
  const docLayers = Array.isArray(doc.layers) ? doc.layers : []
  const knownLayers = new Set(info.layerIds)
  for (const layer of docLayers) {
    if (layer !== null && typeof layer === 'object' && typeof layer.id === 'string') knownLayers.add(layer.id)
  }
  for (const cell of info.cells) {
    const id = String(cell.id)
  ...
  // ── 顺序（z-order）：drawio 的"谁在上面"就是**单元在 root 里的先后** ──
  //
  // 模型里节点的先后 = 画布上的覆盖顺序，所以"置顶/置底"改的是模型数组顺序；
  // 文件那边必须跟着改，否则重新载入（或 drawio 打开）又变回原来的层叠顺序。
  //
  // 只在**真的变了**的时候才动：读进来时模型的先后就是文件里的先后，
  // 于是"打开后原样保存逐字节不变"这条性质不受影响（下面的 reorderKind 会先比对）。
  const moveEdits = planReorder(info, edits, nodes, edges, owned)
  for (const edit of moveEdits) edits.push(edit)
...
/**
 * 顺序改动 → 编辑计划（把"文件里这些单元的先后"改成模型里的先后）。
 *
 * 为什么不能不做：`置顶/置底`在画布上改的是模型数组顺序，而文件里单元的先后**就是**层叠顺序——
 * 不写回去的话，重新载入或拿 drawio 打开又变回原样，用户会觉得"置顶没生效"。
 *
 * 做法：先算文件里"我们拥有的这一种单元"的 id 序列，和模型里的序列比；一样就**一条编辑都不发**
 * （这条保证了"打开后原样保存逐字节不变"）。不一样时，把位置不对的单元挪到目标位置前面：
 * 删掉原位置 + 在目标行首插入（缩进沿用目标行）。
 *
 * 节点与边**分开**比对：模型里节点和边是两个数组，而文件里它们可能交错排列；
 * 拿 `[...nodes, ...edges]` 去比会把交错的文件整体重排一遍（每次保存都大改，绝对不能这么干）。
 */
function planReorder(info, edits, nodes, edges, owned) {
  const out = []
  const editedText = new Map()
  for (const edit of edits) if (edit.end > edit.start) editedText.set(edit.start, edit.text)
  const deleted = new Set()
  for (const edit of edits) if (edit.end > edit.start && edit.text === '') deleted.add(edit.start)

  const orderOf = (wanted) => {
    const list = []
    for (const cell of info.cells) {
      if (wanted.has(String(cell.id))) list.push(cell)
    }
    return list
  }
  const lineStartOf = (offset) => {
    const at = info.modelXml.lastIndexOf('\n', offset)
    return at < 0 ? 0 : at + 1
  }
  const indentOf = (offset) => info.modelXml.slice(lineStartOf(offset), offset)

  const kinds = [
    { wanted: new Set(nodes.map((n) => String(n.id))), model: nodes.map((n) => String(n.id)) },
    { wanted: new Set(edges.map((e) => String(e.id))), model: edges.map((e) => String(e.id)) },
  ]
  for (const kind of kinds) {
    const fileCells = orderOf(kind.wanted)
    // 只考虑"文件里有、模型里也有"的单元（新单元由 additions 负责；被删的已经没了）。
    const current = fileCells.filter((c) => kind.model.indexOf(String(c.id)) >= 0).map((c) => String(c.id))
    if (current.join('\u0000') === kind.model.join('\u0000')) continue
    // 有删除/新增混在一起时顺序本来就会变，那种情况不折腾（新增在末尾、删除不影响相对顺序）。
    if (fileCells.length !== kind.model.length) continue
    const byId = new Map()
    for (const cell of fileCells) byId.set(String(cell.id), cell)

    // **一次插入**，而不是逐个移动：逐个"插到同一个锚点前面"，最终先后取决于 splice 的应用顺序，
    // 实测会插反（[n1,n2,n3] 置顶 n1 得到 n3,n1,n2）。所以：
    // 把这些单元各自删掉，再在**最早那个单元的行首**按模型顺序整体插回。
    // 中间夹着的、不归我们管的单元不受影响（只是位置跟着挪）。
    const texts = []
    let firstAt = -1
    for (const id of kind.model) {
      const cell = byId.get(id)
      if (cell === undefined) continue
      texts.push(indentOf(cell.start) + (editedText.has(cell.start) ? editedText.get(cell.start) : info.modelXml.slice(cell.start, cell.end)))
      if (deleted.has(cell.start) === false) out.push({ start: cell.start, end: cell.end, text: '' })
      if (firstAt < 0 || cell.start < firstAt) firstAt = cell.start
    }
    if (firstAt < 0 || texts.length === 0) continue
    const at = lineStartOf(firstAt)
    out.push({ start: at, end: at, text: texts.join('\n') + '\n' })
  }
  void owned
  return out
}
...
export function buildMxfile(doc, options) {
  ...
  const dropped = []
  const lines = []
  // 纸张：模型里带了就照它写（读进来是什么尺寸，从零重建时还是什么尺寸），没带就用 drawio 的缺省。
  const pageW = doc.page !== undefined && doc.page !== null && Number.isFinite(Number(doc.page.w)) && Number(doc.page.w) > 0 ? Number(doc.page.w) : PAGE_W
  const pageH = doc.page !== undefined && doc.page !== null && Number.isFinite(Number(doc.page.h)) && Number(doc.page.h) > 0 ? Number(doc.page.h) : PAGE_H
