...
判据是 `|dx| <= |dy|` 时先横（即"先走跨度更大的一轴"），但**它不知道我们从哪儿来**，
所以还有一条覆盖规则：**不许原路折返**。横着走进 `a` 之后，若 `b` 在反方向，
再"先横"就等于把上一段重新描一遍（出去又回来 = 两段重合的线）；
这时改走先竖后横，竖的那一步是新方向，绝不会压在上一段上。竖着进来同理。
...
选 L 只能防住"这一拐要不要折返"，防不住**被数据逼出来的**折返：
目标侧的桩点（`stubPointFor`，边框外 24px）与用户折点分居拐点两侧时，
无论先横还是先竖都要走一段回程。真机截图就是这样来的 ——
...
上面那条真机走线的**逐点期望值**、以及"同一判据在旧走线上能认出 1 段折返"（防假绿）。
...
```sh
npm test        # 120 项 mxfile 编解码/写回（含顺序/数据/标签单元）+ 172 项路由/自环/迟滞/折返 + 134 项宿主 + 234 项渲染/剪贴板/把手 + 71 项组件（合计 731）
```
