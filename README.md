...
| 客户端 | **线上的文字**（边自己的 `value`）：双击线、线上的文字、或线上的把手都能就地改（右键也有「改标签」；AI 侧是 `addEdge {label}` / `setLabel`）；**按住线上的字就能拖**（位置按 drawio 的存法写进边几何的 `x/y/offset`，右键「标签居中」放回中点）；落点缺省是 drawio 的**弧长中点**（`mxGraphView.updateEdgeLabelOffset` + `getPoint`），文字与底衬都挂在同一条边的交互面上 —— 否则它们画在 12px 命中带上面，会把双击吃掉 |
...
```sh
npm test        # 137 项 mxfile 编解码/写回（含顺序/数据/标签单元/标签位置）+ 176 项路由/自环/迟滞/折返 + 134 项宿主 + 266 项渲染/标签拖动/剪贴板/把手 + 85 项组件（合计 798）
```
