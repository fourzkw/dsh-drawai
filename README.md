...
| 双方 | **字号可改**：节点标签、独立文字、连线上的文字都吃 drawio 的 `fontSize` 键。右键菜单里「默认 / 10 / 12 / 14 / 18 / 24」一行（三个地方共用同一个控件），「默认」= **删键**回各自缺省（节点/文字 12、连线 10），不留 `fontSize=0` 之类噪音；AI 侧就是 `fontSize:18`（`null` = 删键） |
...
```sh
npm test        # 155 项 mxfile 编解码/写回（含顺序/数据/标签单元/标签位置/图层/独立文字）+ 177 项路由/自环/迟滞/折返 + 221 项宿主（聚焦/注入/树布局/坐标/move/独立线/highlight/撤回/图层/独立文字/线型与字号）+ 425 项渲染/标签/挖空/框选/菜单当前值/辅助线/批量样式/全选/图层/字段不丢/双击改标签/独立文字/线型与字号 + 177 项组件（菜单真实求值/点外面关掉/指针捕获策略/一类一行下拉/独立文字入口/线型与字号入口/降级边界）（合计 1155）
