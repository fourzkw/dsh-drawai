...
**未实现**（用户侧）：设置页、**图层**（新建/隐藏/锁定/切换：文件里的图层结构目前原样保留，
...
```sh
npm test        # 137 项 mxfile 编解码/写回（含顺序/数据/标签单元/标签位置）+ 176 项路由/自环/迟滞/折返 + 168 项宿主（聚焦/注入/树布局/坐标/move/独立线/highlight/撤回）+ 319 项渲染/标签/挖空/框选/辅助线/批量样式/全选 + 86 项组件（合计 886）
```
