...
`emptyDoc()`（新建画布）也带一个缺省图层 —— 与盘上那个 `<mxCell id="1" parent="0" />` 对应，
落盘字节完全相同，只是让"新建后"与"存过再打开"的内存形状一致（否则新建的画布在图层面板里
...
> ③ **"逐个字段抄文档"没人管。** 图层在**四处**被同一个毛病吃掉：宿主的 `normalizeDoc`、
> 客户端的 `docFromPayload`（宿主 → 客户端）、`cloneDoc`（每次本地改动与撤销快照）、
> `pasteInto`（粘贴）—— 都是 `{ version, meta, nodes, edges, labels }` 这么一个个列出来的，
> 于是 `layers` 一路被丢。用户看到的是"这张画布没有图层信息"，而**更隐蔽的是显示/隐藏变成空操作**：
...
```sh
npm test        # 153 项 mxfile 编解码/写回（含顺序/数据/标签单元/标签位置/图层）+ 176 项路由/自环/迟滞/折返 + 190 项宿主（聚焦/注入/树布局/坐标/move/独立线/highlight/撤回/图层）+ 347 项渲染/标签/挖空/框选/辅助线/批量样式/全选/图层/字段不丢 + 103 项组件（菜单真实求值/降级边界）（合计 969）
```
