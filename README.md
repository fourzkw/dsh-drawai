...
- **拖拽时只吸网格，不与其它节点对齐、也不画对齐辅助线**（drawio 的 guides）；
  对齐只能事后用右键「对齐/分布」。
- **多选后不能批量改样式**：形状/配色/线型的菜单项作用于**右键点中的那一个**
  （删除、拖动、复制、对齐/分布是整组生效的）。
- **没有 `Ctrl+A` 全选**，也没有方向键微移（位置只能靠鼠标拖）。
- **单条选中的连线不能复制**：剪贴板只收"两端都在选区内"的边（设计如此，
  否则粘出来的边会指向没被粘贴的节点）。
- 导出只有 SVG 与 PNG(2×)；没有 PDF，也没有"只导出选中部分"。

**未实现**（AI 侧，都是这轮真机操作时暴露出来的）：

- **没有"移动已有节点"的 op**：`addNode` 收 `x/y`，但 `setStyle` 不收 ——
  所以"把这几个节点往右挪 40px""把 A 对齐到 B"只能删掉重画（会丢 id、丢边上的
  端点约束与折点）。要补一个 `{op:'move', id, dx, dy}`（或让 `setStyle` 也收 `x/y/w/h`）。
- **`diagram_read` 不回节点坐标**（只有 id/label/shape/style）：AI 看不见位置，
  也就判断不了"是否重叠""是否对齐""挪了多少"。补 `x/y/w/h` 是个很小的改动。
- **AI 改完不能撤销**：服务端推来新版本时客户端会 `resetHistory()`（故意的 ——
  否则一次撤销会把 AI 刚写的改动悄悄顶掉），所以 Ctrl+Z 对 AI 的改动无效。
  要支持得在宿主留一版快照/diff，或者加一个「撤销这次 AI 改动」的入口。
- **AI 不能在画布上高亮/选中某个东西给你看**：现在只有客户端→宿主的聚焦上报，
  反向没有通道（"你看这几个节点"这种表达目前只能靠读回来念 id）。
